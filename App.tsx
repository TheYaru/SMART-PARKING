...
// Definición de interfaces para tipar el estado del estacionamiento
...
export default function App() {
  // Estados para manejar la vista, el estado del estacionamiento, la carga y el pago
  const [view, setView] = useState<'user' | 'admin'>('admin');
  const [status, setStatus] = useState<ParkingStatus | null>(null);
  const [loading, setLoading] = useState(true);
  const [paying, setPaying] = useState<number | null>(null);

  // Función para obtener el estado del estacionamiento desde la API
  ...
  // Función para procesar el pago de un espacio
  ...
  // Efecto para obtener el estado inicial y configurar un intervalo de actualización
  useEffect(() => {
    fetchStatus();
  ...
  // Renderizado principal con navegación y vistas condicionales
  return (
    <div className="min-h-screen bg-[#0A0A0B] text-white font-sans selection:bg-[#F27D26]/30">
      ...
      {/* Contenedor principal que cambia según la vista seleccionada */}
      <div className="pl-16">
        {view === 'admin' ? (
...
function AdminDashboard({ status, loading }: { status: ParkingStatus | null, loading: boolean }) {
  return (
    ...
    >
      {/* Encabezado con título y última sincronización */}
...
// Componente para la vista de usuario
